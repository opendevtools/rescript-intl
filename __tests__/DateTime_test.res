...
describe("Swedish", () => {
  ...
  test("formats string date", () => {
    makeFromString(~date="2020-03-18", ~locale=Some("sv-SE"), ()) |> expect |> toEqual("2020-03-18")
  })

  testAll("weekday", list{(#long, "onsdag"), (#short, "ons"), (#narrow, "O")}, ((
    weekday,
    expected,
  )) => {
    formatter(~options=Options.make(~weekday=Some(weekday), ()), ()) |> expect |> toEqual(expected)
  })
